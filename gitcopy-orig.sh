#!/bin/bash
#################################################################
#  Nextion Screen Support for Nextion 2.4, 3.5  and 7.0" Screen	#
...
#   	sudo git clone --depth 1 https://github.com/TGIF-Network/NX8048K070-KDO-Upgrade /home/pi-star/Nextion_Temp
	status="$?"
	if [ "$status" -eq 0 ]; then
		Logit "Git Clone OK"
	else
...
if [ "$scn" == "NX8048P070" ]; then
	...
	#Backup Nextion_Temp
			if [ "$fb" ]; then
                        	echo "Downloaded new EA7KDO Upgrade Screen package for $model$tft"
...
#		cleandirs
		...
		if [ "$Release" == "Beta" ] || [ "$Release" == "Upgrade" ]; then
...
# 		  	sudo git clone --depth 1 https://github.com/TGIF-Network/NX4832K035-KDO-Upgrade /home/pi-star/Nextion_Temp
...
#echo "$scn  - $call" 
if [ "$fb" ]; then
#	        if [ "$scn" != "$s1" -a "$scn" != "$s2" -a  "$scn" != "$s3" -a "$scn" !- "$s4" ]; then
	        if [ "$scn" != "$s1" ] && [ "$scn" != "$s2" ] && [ "$scn" != "$s3" ] && [ "$scn" !- "$s4" ]; then
